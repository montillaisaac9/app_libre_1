...
import com.example.electiva_libre.presentacion.ui.news.NewsVieModel
...
fun ScreenCourses(
    ...
    viewModel: CoursesVieModel = hiltViewModel()
) {

    val snackBarState = remember { SnackbarHostState() }
    val scope = rememberCoroutineScope()
    val response = viewModel.CoursesResponse
    ...
    Scaffold (
        ...
        Column(
            ...
            Row (
                ...
                Text("Cursos", color = MaterialTheme.colorScheme.onBackground, fontSize = 30.sp)
            }
            when(response){
                is ApiResult.Error ->{
                    LaunchedEffect(Unit) {
                        ...
                        LazyColumn(modifier = Modifier.fillMaxSize()) {
                            itemsIndexed(items = list) { index, item ->
                                CardCustom(
                                    ...
                                    color = if (item.isWorkshop) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.secondaryContainer,
                                    title = {
                                        Text(
                                            item.title,
                                            fontSize = 20.sp,
                                            modifier = Modifier.padding(bottom = 20.dp),
                                            color = if (item.isWorkshop) MaterialTheme.colorScheme.onPrimary else MaterialTheme.colorScheme.onTertiary
                                        )
                                    }
                                ) {
                                    CardBodyCourse(
